        ...
        <div class="md-layout-item md-small-size-100">
            <form>
                ...
                <md-card-header class="md-card-header-icon md-card-header-primary">
                    ...
                    <h4 class="title">Registrar categoría</h4>
                </md-card-header>
...
import { CREATE_CATEGORY } from '../../../../plugins/graphql';
 ...
 props: {
  ...
  methods: {
      ...
      initializeFormData: function(){
        ...
        this.formData.append("query","mutation CreateCategory($input: CreateCategoryInput!){createCategory(input: $input){newCategory{id}}}");
      },
      // Función que reemplaza o asigna al formulario una imagen que el usuario cambia
      onFileChange(e) {
      if(this.imageName != '')
      {
    ...
    // Función que crea la imagen en la vista
    ...
    validate() {
      this.$validator.validateAll().then(isValid => {
        if(isValid)
        {
            ...
            {
              this.notifyVue('top','right','warning','No hay una imagen seleccionada');
              return
            }
            this.sending = true;
            ...
            // Función creada para crear la mutación con imágenes en el servidor graphql
            uploadFilesToGraphQLXr(this.formData).then(() => {
              this.notifyVue('top','right','success','Categoría creada correctamente');
  ...
  // Se inicializa el formulario al montar el componente
  mounted(){
