    ...
    <div class="md-layout-item">
         <router-link to="/users/add">
            <md-button style="float: right;"
                ...
                <md-tooltip  md-direction="left">Agregar usuario</md-tooltip>
        ...
        <md-card-content>
          <md-table :value="queriedData" :md-sort.sync="currentSort" :md-sort-order.sync="currentSortOrder" :md-sort-fn="customSort" class="paginated-table table-striped table-hover">
            ...
            <md-table-row slot="md-table-row" slot-scope="{ item }">
              ...
              <md-table-cell md-label="Nombre de usuario">{{ item.username }}</md-table-cell>
              <md-table-cell md-label="Acciones">
                <!-- <router-link to="/users/add">
                    <md-button
                    class="md-button md-just-icon md-theme-default md-info">
                ...
                <router-link :to="'/users/edit/'+item.id">
                    <md-button class="md-just-icon md-warning">
                        <md-tooltip  md-direction="top">Editar información</md-tooltip>
                        <md-icon>create</md-icon>
                    </md-button>
                </router-link>
                 <router-link :to="'/users/password_edit/'+item.id">
...
import { ALL_USERS} from '../../../plugins/graphql';
...
export default {
  ...
  data() {
    return {
      ...
      footerTable: ["Nombre", "Correo electrónico", "Nombre de usuario", "Acciones"],
      searchQuery: "",
      propsToSearch: ["name", "email", "username"],
      tableData: [],
      searchedData: [],
      fuseSearch: null
    };
  ...
  methods: {
    ...
    // Función que setea los usuarios para ser visualizados en la tabla
    setUsers: function(allUsers){
        for(var i=0;i<allUsers.edges.length;i++)
        {
            this.tableData.push({
                id: allUsers.edges[i].node.id,
                username: allUsers.edges[i].node.username,
                name: allUsers.edges[i].node.firstName+' '+allUsers.edges[i].node.lastName,
                email: allUsers.edges[i].node.email,
            })
        }
    },
    customSort(value) {
